...
모두먼트 회원가입 프로세스를 리뉴얼 하기로 했다. 하게된 이유는 회원가입시 사업자 등록증을 업로드 하고 그 와 관련된 정보들을 서버에 저장하기 위함이였다. 

기술이사님의 요구 사항은 다음과 같았다. 

- 사업자 등록증을 업로드 하고 내려온 정보들 사용자에게 보여줄것.

  > 사업자 번호, 회사 주소, 회사 명, 대표자 명, 법인사업자 여부, 법인 번호, 회사 전화번호.

- 사업자 등록증이 없는 고객일 경우, 사업자 번호 대신 주민등록 번호를 입력 받게 할것. ( 이는 나중에 회사 정책상 사업자 등록증이 없는 고객은 안받기로 결정되어 사라짐. )  
  

- 담당자 정보, 세금계산서 정보 입력받을것. (세금 계산서 정보는 담당자 정보로 동기화 할 수 있게 할것.)

  > 담당자 이름, 담당자 전화번호, 부서 및 직책, 담당자 이메일, 세금계산서 메일주소, 세금 계산서 전화번호.

- 네이버 StmartStore 연동 API 사용하여 연동시킬 것.
...
1. UI와 UX를 어떤 형태로 할 것인가.
  ...
  > 사업자 등록증을 업로드 할때의 로딩 화면은 어디에서 보여줄까?    


2. 추후에 다른 스토어가 추가되던지 했을 때 가장 ```확장성``` 있게 하려면 어떤 구조로 코드를 작성해야 할까?

3. 가장 집약적으로 Validation 을 작성할 수 있는 방법은 무엇일까? 

  > 각각의 Input에서 validation을 정하지 말고 하나의 함수에서 모든 validation을 처리 할 수는 없을까?     


↓ 고심 끝에 결정한 디자인.

<img width="525" alt="스크린샷 2022-07-29 오전 9 51 58" src="https://user-images.githubusercontent.com/87749134/181660587-18814db4-bb4a-4a34-a40d-b1cf79959d6f.png">
